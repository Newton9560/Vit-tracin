...
class MyDataSet(Dataset):
    ...
    def __init__(self, images_path: list, images_class: list, transform=None):
        ...
        self.transform = transform  #图片数据transform变换

    def __len__(self):
        return len(self.images_path)

    def __getitem__(self, item):
        img = Image.open("F:\paper\PASCALVOC\VOCdevkit_t_v\VOC2012\JPEGImages\\" + self.images_path[item])
        # RGB为彩色图片，L为灰度图片
        if img.mode != 'RGB':
...
class MyDataSet_CIFAR(Dataset):
    ...
    def __getitem__(self, item):
        ...
        path = Path(fileName)
        label = int(self.classification[str(path.parts[3])])
...
class MyDataSet_CIFAR_Tracin(Dataset):
    """自定义数据集"""
    ...
    def collate_fn(batch):
        # 官方实现的default_collate可以参考
        # https://github.com/pytorch/pytorch/blob/67b7e751e6b5931a9f45274653f4f653a4e6cdf6/torch/utils/data/_utils/collate.py
        images, labels, fileName = tuple(zip(*batch))
